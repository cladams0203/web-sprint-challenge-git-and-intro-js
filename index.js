...
function listOfNames(arry) {
  return arry.map((x) => x["name"]);
}
...
function removeArtist(arry, num) {
  const newArry = [...arry];
  newArry.splice(num, 1);
  return newArry;
}
...
function addArtist(arry, name, years, genre, nationality, bio) {
...
function lotsOfArt(arry) {
  return arry.filter((x) => x.paintings >= 100).map((y) => y.name);
}
...
function artistInfo(arry, name) {
  return arry.find((x) => x.name === name).bio;
}
...
function artistByCountry(arry, nat) {
  const newArry = [];
  arry.forEach((x) => {
    const natArray = x.nationality.split(",");
    if (natArray.length === 1 && x.nationality.includes(nat)) {
      newArry.push(x.name);
    }
  });
  return newArry;
